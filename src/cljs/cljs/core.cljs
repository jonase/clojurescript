...
(defn str
  ...
  ([] "")
  ([x] (cond
        (symbol? x) (.substring x 2 (alength x))
        (keyword? x) (str* ":" (.substring x 2 (alength x)))
        (nil? x) ""
        :else (.toString x)))
  ([x & ys]
     ((fn [sb more]
...
(set! (.-EMPTY Vector) (Vector. nil (array) 0))

(set! (.-fromArray Vector) (fn [xs] (Vector. nil xs nil)))
    ...
    (cond
     ...
     (< 1 (- cnt (tail-off coll)))
      ...
      :else (let [new-tail (array-for coll (- cnt 2))
                  ...
                  new-root (if (nil? nr) (.-EMPTY_NODE PersistentVector) nr)
                  cnt-1 (dec cnt)]
...
(set! (.-EMPTY_NODE PersistentVector) (pv-fresh-node nil))
(set! (.-EMPTY PersistentVector) (PersistentVector. nil 0 5 (.-EMPTY_NODE PersistentVector) (array) 0))
(set! (.-fromArray PersistentVector)
      (fn [xs no-clone]
        (let [l (alength xs)
              xs (if (identical? no-clone true) xs (aclone xs))]
          (if (< l 32)
            (PersistentVector. nil l 5 (.-EMPTY_NODE PersistentVector) xs nil)
            (let [node (.slice xs 0 32)
                  v (PersistentVector. nil 32 5 (.-EMPTY_NODE PersistentVector) node nil)]
...
(defn- obj-map->hash-map [m k v]
  (let [ks  (.-keys m)
        len (alength ks)
        so  (.-strobj m)
        out (with-meta (.-EMPTY PersistentHashMap) (meta m))]
    (loop [i   0
           out (transient out)]
      (if (< i len)
        (let [k (aget ks i)]
...
(set! (.-HASHMAP_THRESHOLD ObjMap) 32)
...
(deftype HashMap [meta count hashobj ^:mutable __hash]
  ...
  IEmptyableCollection
  (-empty [coll] (with-meta (.-EMPTY HashMap) meta))
...
(set! (.-EMPTY HashMap) (HashMap. nil 0 (js-obj) 0))

(set! (.-fromArrays HashMap) (fn [ks vs]
  (let [len (alength ks)]
    (loop [i 0, out (.-EMPTY HashMap)]
      (if (< i len)
...
(deftype PersistentArrayMap [meta cnt arr ^:mutable __hash]
  ...
  (-assoc [coll k v]
    (let [idx (array-map-index-of coll k)]
      (cond
        (== idx -1)
        (if (< cnt (.-HASHMAP_THRESHOLD PersistentArrayMap))
          ...
          (persistent!
           (assoc!
            (transient (into (.-EMPTY PersistentHashMap) coll))
            k v)))
...
(set! (.-HASHMAP_THRESHOLD PersistentArrayMap) 16)

(set! (.-fromArrays PersistentArrayMap)
      (fn [ks vs]
        (let [len (count ks)]
          (loop [i   0
  ...
  (-assoc! [tcoll key val]
    (if editable?
      (let [idx (array-map-index-of tcoll key)]
        (if (== idx -1)
          (if (<= (+ len 2) (* 2 (.-HASHMAP_THRESHOLD PersistentArrayMap)))
            (do (set! len (+ len 2))
                (.push arr key)
                (.push arr val)
                tcoll)
...
(set! (.-fromArrays PersistentHashMap)
      (fn [ks vs]
        (let [len (alength ks)]
          (loop [i 0 out (transient (.-EMPTY PersistentHashMap))]
            (if (< i len)
              (recur (inc i) (assoc! out (aget ks i) (aget vs i)))
              (persistent! out))))))
...
(set! (.-EMPTY PersistentHashSet) (PersistentHashSet. nil (hash-map) 0))
...
(set! (.-EMPTY PersistentTreeSet) (PersistentTreeSet. nil (sorted-map) 0))
...
(set! (-> MultiFn .-prototype .-call)
      (fn [_ & args]
        (this-as self
          (-dispatch self args))))

(set! (-> MultiFn .-prototype .-apply)
      (fn [_ args]
        (this-as self
          (-dispatch self args))))
